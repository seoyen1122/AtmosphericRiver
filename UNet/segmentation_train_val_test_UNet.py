...
test_dir = "/data0/aix23606/soyeon/climatenet_full_test" 

# 모든 .nc 파일 로드
all_files = sorted(glob(os.path.join(train_dir, '*.nc')))
# 연도 기준으로 train/val 분할
train_files = [f for f in all_files if int(os.path.basename(f).split('-')[1]) <= 2004]
val_files   = [f for f in all_files if int(os.path.basename(f).split('-')[1]) >= 2005]

test_files = sorted(glob(os.path.join(test_dir, '*.nc')))

# ✅ 데이터 로딩 (기존 데이터 로딩 함수 재사용)
selected_vars = ['U850', 'V850', 'PSL', 'TMQ']
X_train, y_train = load_nc_data(train_files, selected_vars)
X_val, y_val = load_nc_data(val_files, selected_vars)

X_test, y_test = load_nc_data(test_files, selected_vars)  

# ✅ 라벨 이진화
y_train_bin = tf.cast(tf.where(y_train == 2, 1, 0), tf.uint8)
y_val_bin = tf.cast(tf.where(y_val == 2, 1, 0), tf.uint8)

y_test_bin = tf.cast(tf.where(y_test == 2, 1, 0), tf.uint8)  

# ✅ 학습용 Dataset
train_dataset = create_tf_dataset(X_train, y_train_bin, batch_size=4)
val_dataset = create_tf_dataset(X_val, y_val_bin, batch_size=4)

test_dataset = create_tf_dataset(X_test, y_test_bin, batch_size=4) 
...
# ✅ 학습 진행
history = model.fit(train_dataset, validation_data=val_dataset, epochs=600)
...
# ✅ 시각화 및 저장
output_dir = "/home/aix23606/soyeon/ar/result/train_val_test_v1"  # output_dir 경로 수정
...
with open(os.path.join(output_dir, 'test_performance.txt'), 'w') as f:
    f.write(f"Test Loss: {test_loss:.4f}\nTest Accuracy: {test_accuracy:.4f}\n")

# IoU Histogram: 예측 성능 분포 보기
plt.figure(figsize=(6, 4))
plt.hist(iou_scores, bins=10, range=(0,1), color='skyblue', edgecolor='black')
...
# Mean Prediction Map: 예측된 평균 마스크 시각화
mean_pred = np.mean(y_pred_subset, axis=0)
plt.figure(figsize=(6, 4))
plt.imshow(mean_pred[:, :, 0], cmap='hot', interpolation='nearest')
plt.colorbar(label='Mean Prediction')
plt.title('Mean Predicted Probability Map (subset)')
plt.axis('off')
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'mean_pred_map.png'))
plt.close()
